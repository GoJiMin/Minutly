...
import {Button, Card, CardContent, CardHeader, Separator, Text} from '@/shared/components';
...
export function InterruptionAlertPanel({interruptions, onMarkInterruptionReviewed, onMoveToInterruption}: Props) {
  ...
  return (
    <Card
      size="sm"
      className="shrink-0 gap-3 rounded-lg border border-border bg-background mx-3 md:mx-0 py-3 shadow-none ring-0"
    >
      <CardHeader>
        <Text className="font-medium leading-6 text-sm md:text-base">녹음 중단 구간 {interruptionCount}개</Text>
        <Text variant="small" className="text-muted-foreground hidden md:block">
            ...
            return (
              ...
              >
                <Button
                  ...
                  className="text-xs md:text-sm rounded-none px-3 text-foreground"
                ...
                <Button
                  ...
                  className="text-xs md:text-sm rounded-none px-3 text-muted-foreground hover:text-foreground disabled:bg-muted disabled:text-muted-foreground"
                  disabled={isReviewed}
